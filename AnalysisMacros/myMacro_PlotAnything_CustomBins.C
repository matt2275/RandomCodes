...
void myMacro_PlotAnything_CustomBins(std::string config_file_name, std::string list_select = "list_selections.txt", std::string list_data = "list_datatypes.txt"){
    string file_line;
...
string cms_text = "PbPb #sqrt{s_{NN}} = 5.02 TeV ( " + stream.str() +  " nb^{-1} )";
  ...
  //   iPos = 10*(alignement 1/2/3) + position (1/2/3 = left/center/right)
   ...
   string out_folder = save_folder +  RJR_Method+"/" +PlotFolderName+ "/" + normalization;
...
TH1F *data ;
    ...
    cout << "num bins before " << num_bins << endl;
   ...
   cout << "Test Ratio  Min " << RG_min << " max " << RG_max << endl;
	h->GetXaxis()->SetTitle("");
