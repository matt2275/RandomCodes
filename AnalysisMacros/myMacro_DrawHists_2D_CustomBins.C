...
string save_folder = "/eos/user/m/mnickel/GenEffStudy/Outputs/WithL1Triggers/";
...
// "Data","DiTau_SuperChic","DiTau_Madgraph","BBbar","CCbar","DiMuon_Gamma","DiElectron_SuperChic","DiElectron_Starlight","DiMuon_noFSR"
 ...
 vector<string> DataTypes = {"DiMuon_Arash"};
 // vector<string> DataTypes = {"DiMuon_noFSR","DiElectron_Starlight","DiElectron_SuperChic","DiMuon_Arash"};
...
void myMacro_DrawHists_2D_CustomBins(std::string config_file_name, std::string list_select = "list_selections.txt", std::string list_data = "list_datatypes.txt"){
    string file_line;
...
string cms_text = "#sqrt{s_{NN}} = 5.02 TeV ( " + stream.str() +  " nb^{-1} )";
  ...
  //   iPos = 10*(alignement 1/2/3) + position (1/2/3 = left/center/right)
   ...
   for(string dataType : DataTypes){
    ...
    string out_folder;
   out_folder = save_folder +  RJR_Method+ "/" +selection+ "/" + Global_dataType;
...
auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[Global_dataType];
   TFile file_NOM(FileName,"READ");
    cout << FileName << " "  << hist_name << endl;
    TString hist_name_2D = "Hist2D_" + hist_name;
    TH2F *h = static_cast<TH2F*>(file_NOM.Get(hist_name_2D)->Clone());
