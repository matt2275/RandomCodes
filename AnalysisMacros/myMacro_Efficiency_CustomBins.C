...
void myMacro_Efficiency_CustomBins(std::string config_file_name, std::string list_select = "list_selections.txt", std::string list_data = "list_datatypes.txt"){
    string file_line;
    ...
    if(config_inputs.size() < 9){ cout << " Bad config file " << endl; return;}
...
string cms_text = "#sqrt{s_{NN}} = 5.02 TeV ( " + stream.str() +  " nb^{-1} )";
  ...
  //   iPos = 10*(alignement 1/2/3) + position (1/2/3 = left/center/right)
   string out_folder;
   TString eff_folder = "Efficiency_" + selection + "_OVER_" + DENOM;
...
h->GetXaxis()->SetNdivisions(6,5,0);
  ...
  // }
    ...
    for(string dataType : DataTypes){
    auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[dataType];
    TFile file_NOM(FileName,"READ");
    TH1F *tmp_hist_NOM = static_cast<TH1F*>(file_NOM.Get(hist_name)->Clone());
    TString FileName_DENOM = replace_first(FileName,Global_selection, DENOM);

    TFile file_DENOM(FileName_DENOM,"READ");
    TH1F *tmp_hist_DENOM = static_cast<TH1F*>(file_DENOM.Get(hist_name)->Clone());
    // TH1F *tmp_hist = (TH1F*)tmp_hist_Pass->Clone("tmp_hist");
    tmp_hist_DENOM->Sumw2();
    tmp_hist_NOM->Sumw2();
    // TH1F* tmp_hist = new TH1F("binomial", "Test binomial", num_bins,min_bin,max_bin);
    TEfficiency* tmp_hist = 0;
    tmp_hist = new TEfficiency(*tmp_hist_NOM, *tmp_hist_DENOM);
    tmp_hist->SetStatisticOption(TEfficiency::kFCP);
    tmp_hist->SetConfidenceLevel(.68);
