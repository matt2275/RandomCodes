...
string save_folder = "";
 ...
 // vector<string> DataTypes = {"Data", "DiMuon_noFSR","DiElectron_Starlight","DiElectron_SuperChic","DiMuon_Arash"};
 // vector<string> DataTypes = {"DiElectron_Starlight","DiElectron_SuperChic"};
 vector<string> DataTypes = {};
 
  // if(prong_type == "1Prong" ) selections = { "Muon_All", "Muon_Pass", "Muon_Fail", "Muon_Trigger_All", "Muon_Trigger_Pass", "Muon_Trigger_Fail", "Electron_EG5_All", "Electron_EG5_Pass", "Electron_EG5_Fail", "Electron_EG5_Trigger_All", "Electron_EG5_Trigger_Pass", "Electron_EG5_Trigger_Fail", "Electron_EG3_All", "Electron_EG3_Pass", "Electron_EG3_Fail", "Electron_EG3_Trigger_All", "Electron_EG3_Trigger_Pass", "Electron_EG3_Trigger_Fail","Muon_MuMuCuts_All", "Muon_MuMuCuts_Pass", "Muon_MuMuCuts_Fail", "Muon_MuMuCuts_Trigger_All", "Muon_MuMuCuts_Trigger_Pass", "Muon_MuMuCuts_Trigger_Fail"};
 // if(prong_type == "1Prong" ) selections = {"Has_Pos_Trk"};
 
 // if(prong_type == "3Prong" ) selections = {"El3Prong","Mu3Prong"};
// vector<string>  selections = {"Has_Pos_Trk","Has_Neg_Trk","Has_Pos_Muon","Has_Neg_Muon","Has_Pos_Ele","Has_Neg_Ele","Has_Pos_Muon_SingleMu","Has_Neg_Muon_SingleMu","Has_Pos_Ele_SingleEG3","Has_Neg_Ele_SingleEG3","Has_Pos_Ele_SingleEG5","Has_Neg_Ele_SingleEG5"};
// vector<string>  selections = {"Electron_EG3_Pass","Electron_EG5_Pass"};
vector<string>  selections = {};
 

 
// string DENOM = "Electron_EG3_All"; // denominator of efficiency calculation. selections is nominator

// string SF_NOM_datatype = "Data"; // numerator of scale type. usually data


// string outName_start = "DiElectron_";
...
// "Data","DiTau_SuperChic","DiTau_Madgraph","BBbar","CCbar","DiMuon_Gamma","DiElectron_SuperChic","DiElectron_Starlight","DiMuon_noFSR"
 ...
 // vector<string> DataTypes = {"DiElectron_SuperChic","DiElectron_Starlight"};
 ...
 vector<string> MCTypes = {"DiElectron_SuperChic","DiElectron_Starlight"};
...
TGraphErrors* GetScaleFactor(TEfficiency* hist1, TEfficiency* hist2, string error_type = "down"){
       TH1* copy = hist1->GetCopyTotalHisto();	   
      int nbins = copy->GetNbinsX();
       double X_val[nbins];
       double X_err[nbins];
       double Y_val[nbins];
       double Y_err[nbins];     	   	   
       ...
       double sum;
       for (Int_t i=0; i<=nbins - 1 ;i++) {
        val1 = hist1->GetEfficiency(i + 1);
		  val2 = hist2->GetEfficiency(i + 1);
        if(error_type == "down"){
        err1 = hist1->GetEfficiencyErrorLow(i + 1);
		  err2 = hist2->GetEfficiencyErrorLow(i + 1);           
        }
        else if(error_type == "up"){
        err1 = hist1->GetEfficiencyErrorUp(i + 1);
		  err2 = hist2->GetEfficiencyErrorUp(i + 1);              
        }
        else{
        err1 = 0;
		  err2 = 0;   
        ...
        rel_err2 = pow(err2/val2,2);
		  sum = val1 + val2;
		  X_val[i] = copy->GetXaxis()->GetBinCenter(i+1);
		  X_err[i] = copy->GetXaxis()->GetBinWidth(i+1)/2;
		  Y_val[i] = (val1 /( val2 + epsi));
		  Y_err[i] = (val1 /( val2 + epsi))*pow(rel_err1 + rel_err2, .5);	
	   }
...
TH1* GetScaleFactor_TEST(TEfficiency* hist1, TEfficiency* hist2, string error_type = "down"){
       TH1* copy = hist1->GetCopyTotalHisto();	   
      int nbins = copy->GetNbinsX();
       double X_val[nbins];
       double X_err[nbins];
       ...
       double sum;
       for (Int_t i=0; i<=nbins +1 ;i++) {
        val1 = hist1->GetEfficiency(i);
		  val2 = hist2->GetEfficiency(i);
        if(error_type == "down"){
        err1 = hist1->GetEfficiencyErrorLow(i);
		  err2 = hist2->GetEfficiencyErrorLow(i);           
        }
        else if(error_type == "up"){
        err1 = hist1->GetEfficiencyErrorUp(i);
		  err2 = hist2->GetEfficiencyErrorUp(i);              
        }
        else{
        err1 = 0;
		  err2 = 0;   
        ...
        rel_err2 = pow(err2/val2,2);
		  ...
		  copy->SetBinContent(i,1,(val1 /( val2 + epsi)));
		  copy->SetBinError(i,(val1 /( val2 + epsi))*pow(rel_err1 + rel_err2, .5));	
	   }
...
// histParams["DiElectron_SuperChic"] = {true , Hists_Folder  +  RJR_Method +"/" +selection+"/MinCuts_DiElectron_SuperChic.root" ,880 ,33, Lumi*8.83E6/67810000/2};
// histParams["DiElectron_Starlight"] = {true , Hists_Folder  +  RJR_Method +"/" +selection+"/MinCuts_DiElectron_Starlight.root" ,860 ,32, Lumi*7.92E6/66750000/2};
...
// histParams["DiMuon_Starlight"] = {true , Hists_Folder  +  RJR_Method +"/" +selection+"/MinCuts_DiMuon_Starlight.root" ,860 ,22, Lumi*189.08E3/30000000*17.97 };
...
// histParams["DiMuon_Arash"] = {true , Hists_Folder  +  RJR_Method +"/" +selection+"/MinCuts_DiMuon_Arash.root" ,900 ,23, Lumi*8.83E6/250000/2.5 };
...
// FancyNames["DiMuon_Starlight"] = "#gamma #gamma #rightarrow #mu^{+}#mu^{-} (MC SL)";
// FancyNames["DiMuon_Private"] = "#gamma #gamma #rightarrow #mu^{+}#mu^{-} (MC Private)";
...
void myMacro_ScaleFactors_CustomBins(std::string config_file_name, std::string list_select = "list_selections.txt", std::string list_data = "list_datatypes.txt"){
    string file_line;
 ...
 // if(prong_type == "3Prong" ) selections = {"El3Prong","Mu3Prong"};
 // if(prong_type == "1Prong" ) selections = {"El_NoMass","El_0_5","El_5_10","El_10_15","El_15_20","El_20_30","El_JPsi","Mu_NoMass","Mu_0_5","Mu_5_10","Mu_10_15","Mu_15_20","Mu_20_30","Mu_JPsi"};
 // if(prong_type == "1Prong" ) selections = {"Muon_MuMuCuts_All","Muon_MuMuCuts_Trigger_All","Muon_All","Muon_Trigger_All"};
 // if(prong_type == "1Prong" ) selections = {"Has_Pos_Trk","Has_Neg_Trk", "Has_Pos_Muon","Has_Neg_Muon", "Has_Pos_Ele","Has_Neg_Ele","Has_Pos_Muon_SingleMu","Has_Neg_Muon_SingleMu",  "Has_Pos_Ele_SingleEG3","Has_Neg_Ele_SingleEG3", "Has_Pos_Ele_SingleEG5","Has_Neg_Ele_SingleEG5"};
 // if(prong_type == "1Prong" ) selections = {"Has_Neg_Ele_SingleEG3", "Has_Neg_Ele_SingleEG5"};
 // if(prong_type == "1Prong" ) selections = { "Electron_EG5_All", "Electron_EG5_Trigger_All",  "Electron_EG3_All","Electron_EG3_Trigger_All"};
 if(prong_type == "3Prong" )RJR_Methods ={"HasVisMass"};
 if(prong_type == "1Prong" )RJR_Methods ={"HasVisMass"};
 
 
 
   string CSV_file = paraTree_filename;
TTree *ParameterTree = new TTree("ParameterTree", "tree to fill hist parameters");
ParameterTree->ReadFile(CSV_file.c_str(),"KeepHists/I:BranchName/C:HistName/C:HistTitle/C:CustomBinsName/C:nBins/I:min/D:max/D:xaxis/C:yaxis/C:label_pos/I:x_leg_max/D:x_leg_width/D:y_leg_max/D:y_leg_width/D");
int KeepHists;
...
string cms_text = "#sqrt{s_{NN}} = 5.02 TeV ( " + stream.str() +  " nb^{-1} )";
  ...
  //   iPos = 10*(alignement 1/2/3) + position (1/2/3 = left/center/right)
   ...
   out_folder = save_folder+  RJR_Method+ "/" +eff_folder;
...
{ 
 ...
 // TMultiGraph  *h  = new TMultiGraph();
  ...
  h->GetYaxis()->SetTitle(Eff_y_title);
...
h->GetXaxis()->SetNdivisions(6,5,0);
  ...
  // if(DoLegend){
    ...
    // float xx_ = x_l[0];
    // float yy_ = y_l[0];
    // latex.DrawLatex(xx_+1.*bwx_,yy_,FancyNames["Data"]);
		
    // for( string MCType : DataTypes){
       // // if(MCType == "Data") continue;
    // // bool isMC;
    // // string FileName;
    // // int fill_color ;
    // // int marker_style;
    // // float scale;
    // // isMC, FileName, fill_color, marker_style, scale = histParams[MCType];
    // auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[MCType];
    // if( !isMC) continue;
    // cout << FileName << endl;
    // yy_ -= gap_;
    ...
    // box_.SetLineColor(fill_color);
    // box_.SetFillColor( fill_color );
    // box_.DrawBox( xx_-bwx_/2, yy_-bwy_/2, xx_+bwx_/2, yy_+bwy_/2 );
    // box_.SetFillStyle(1001);
    // box_.DrawBox( xx_-bwx_/2, yy_-bwy_/2, xx_+bwx_/2, yy_+bwy_/2 );
    // latex.DrawLatex(xx_+1.*bwx_,yy_,FancyNames[MCType]);
    ...
    // // box_.SetFillColor( kBlue );
    // // box_.DrawBox( xx_-bwx_/2, yy_-bwy_/2, xx_+bwx_/2, yy_+bwy_/2 );
    // // box_.SetFillStyle(1001);
    // // box_.DrawBox( xx_-bwx_/2, yy_-bwy_/2, xx_+bwx_/2, yy_+bwy_/2 );
    // // latex.DrawLatex(xx_+1.*bwx_,yy_,"#gamma #gamma #rightarrow e^{+}e^{-} (MC)");
  ...
  // }
    ...
    // Observed data
    pad1->cd();
    // using name _Data for nom of SF because data is almost always nominator
    auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[SF_NOM_datatype];
    TFile file_NOM_Data(FileName,"READ");
    TH1F *tmp_hist_NOM_Data = static_cast<TH1F*>(file_NOM_Data.Get(hist_name)->Clone());
    TString FileName_DENOM_Data = replace_first(FileName,Global_selection, DENOM);

    TFile file_DENOM_Data(FileName_DENOM_Data,"READ");
    TH1F *tmp_hist_DENOM_Data = static_cast<TH1F*>(file_DENOM_Data.Get(hist_name)->Clone());
    // TH1F *tmp_hist = (TH1F*)tmp_hist_Pass->Clone("tmp_hist");
    tmp_hist_DENOM_Data->Sumw2();
    tmp_hist_NOM_Data->Sumw2();
    TEfficiency* tmp_hist_Data = 0;
    tmp_hist_Data = new TEfficiency(*tmp_hist_NOM_Data, *tmp_hist_DENOM_Data);
    
    for(string dataType : DataTypes){
    // using name _MC for denom of SF because data is almost always denominator
    auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[dataType];
    TFile file_NOM_MC(FileName,"READ");
    TH1F *tmp_hist_NOM_MC = static_cast<TH1F*>(file_NOM_MC.Get(hist_name)->Clone());
    TString FileName_DENOM_MC = replace_first(FileName,Global_selection, DENOM);

    TFile file_DENOM_MC(FileName_DENOM_MC,"READ");
    TH1F *tmp_hist_DENOM_MC = static_cast<TH1F*>(file_DENOM_MC.Get(hist_name)->Clone());
    // TH1F *tmp_hist = (TH1F*)tmp_hist_Pass->Clone("tmp_hist");
    tmp_hist_DENOM_MC->Sumw2();
    tmp_hist_NOM_MC->Sumw2();
    TEfficiency* tmp_hist_MC = 0;
    tmp_hist_MC = new TEfficiency(*tmp_hist_NOM_MC, *tmp_hist_DENOM_MC);


    auto tmp_hist = GetScaleFactor_TEST(tmp_hist_Data,tmp_hist_MC);
    ...
    tmp_hist->Draw("sameAP");
    legend->AddEntry(tmp_hist,FancyNames[dataType]);
    if(tmp_hist->GetMaximum() > SF_max) SF_max = tmp_hist->GetMaximum();
    if(tmp_hist->GetMinimum() < SF_min) SF_min = tmp_hist->GetMinimum();
    // h->Add(tmp_hist);
    file_NOM_MC.Close();
    file_DENOM_MC.Close();
    }
    file_NOM_Data.Close();
    file_DENOM_Data.Close();
    
     // h->SetMaximum(SF_max*1.2);
     // h->SetMinimum(SF_min*8);
     h->SetMaximum(1.5);
     h->SetMinimum(.5);
     legend->Draw();
