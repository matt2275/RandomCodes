...
void myMacro_Efficiency_2D_CustomBins(std::string config_file_name, std::string list_select = "list_selections.txt", std::string list_data = "list_datatypes.txt"){
...
auto &[isMC, FileName, fill_color, marker_style, scale]  = histParams[Global_dataType];
   TFile file_NOM(FileName,"READ");
    ...
    TH2F *tmp_hist_DENOM = static_cast<TH2F*>(file_DENOM.Get(hist_name_2D)->Clone());
    h->Divide(tmp_hist_DENOM);
